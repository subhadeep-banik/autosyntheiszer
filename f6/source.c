...
void enc_s02_n02(uint8_t * inputb, uint8_t * key,uint8_t * outputb );
...
void bs_expand_keyf6(word_t (* rk)[128], uint8_t * key);
void bs_cipherf6(word_t state[128], word_t (* rk)[128]);
void bs_cipher_revf6(word_t state[128], word_t (* rk)[128]);
void dump_hex(uint8_t * h, int len);
void dump_word(word_t * h, int len);
void dump_block(word_t * h, int len);
void enc_s02_n02(uint8_t * inputb , uint8_t * key,uint8_t * outputb )
{
    word_t input_space[128];
    word_t rk[15][128];
    size_t size=16;
    memset(outputb,0,size);
    word_t * state = (word_t *)outputb;
    bs_expand_keyf6(rk, key);
    while (size > 0)
    {
        if (size < (128 * 64 / 8))
        {
            memset(input_space,0,(128 * 64 / 8));
            memmove(input_space, inputb, size);
            bs_cipherf6(input_space,rk);
        ...
        {
            memmove(state,inputb,(128 * 64 / 8));
            bs_cipherf6(state,rk);
            size -= (128 * 64 / 8);
            state += (128 * 64 / 8);
        }
    }
