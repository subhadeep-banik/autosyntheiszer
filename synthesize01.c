...
{      
      ...
      if (strncmp(in,"aead",4)==0)  return ENCRYPTION;    
      if (strncmp(in,"hashing",7)==0)return HASHING;
      
      if (strncmp(in,"signature",9)==0)  return SIGNATURE;    
      if (strncmp(in,"kem",3)==0)return KEM;   
...
{  
   ...
   plot=0;
    ...
    FILE *f1=fopen(df,"rb"); 
       ...
       if(f1!=NULL){
       while (fgets(string, sizeof(string), f1)) {   
      ...
      if(strncmp((tmp),"throughput:",11)==0)        {tp=atoi(tmp1); /* printf("%s\n",df);*/}
      if(strncmp((tmp),"energy:",7)==0)             {en=atoi(tmp1);}// printf("%s\n",tf); }  
      if(strncmp((tmp),"primitive:",10)==0)         {fn =argex(tmp1); }// printf("%s\n",pf);}
