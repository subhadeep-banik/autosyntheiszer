...
{  
   ...
   while (fgets(string, sizeof(string), f)) {   
      ...
      if(strncmp((tmp),"cycles",6)==0)   Dict[i].cycles=atof(tmp1);  
  ...
  for(j=0;j<count;j++)if(Dict[j].ifunc == fn && Dict[j].slevel == slevel )
  {
     ...
     if(maxt< Dict[j].cycles)maxt= Dict[j].cycles;
     if(mint> Dict[j].cycles)mint= Dict[j].cycles;  
  }
  bp.cycles =  (tp==0) * mint + (tp==1) * ((mint+maxt)/2.0 ) + (tp==2)* maxt;
  ...
  for(j=0;j<count;j++)
  {
     if(Dict[j].ifunc == fn && Dict[j].slevel == slevel )
        ...
        dist[j] = pow((bp.cycles - Dict[j].cycles)/ (maxt-mint), 2)+pow((bp.energy - Dict[j].energy)/ (maxe-mine), 2) ;
  ...
  puts(Dict[id].name); 
    ...
    if(plot){
      ...
      sprintf( com, "set object circle at first %f,%f size scr 0.01 fillcolor rgb 'green’ fillstyle solid",bp.energy,bp.cycles);
      sprintf( com1,"set object circle at first %f,%f size scr 0.01 fillcolor rgb 'blue’ fillstyle solid",Dict[id].energy,Dict[id].cycles);
      sprintf( com2,"plot 'data.temp' using (%f):(%f):($1-%f):($2-%f) with vectors nohead, \
                                           'data.temp' with points pt \"\\U+2299\"",bp.energy,bp.cycles,bp.energy,bp.cycles);
    ...
    for (i=0; i < count; i++)
    {
    fprintf(temp, "%f %f \n", Dict[i].energy, Dict[i].cycles); //Write the data to a temporary file
    }
